...
We randomly split the objects into training, hold-out validation, and
...
To test how this model performs across different, related problems,
...
\end{equation}
where $s=1\,000$ is the bin length.
...
\begin{figure}
  ...
  \caption{Galaxy completeness and star purity values as functions
           of the $i$-band magnitude
           as estimated by the kernel density estimation (KDE) method
           for the differential counts in the CFHTLenS data set.
...
\begin{figure}
  ...
  \caption{
    ...
    magnitude for the integrated counts in the CFHTLenS data set.
...
\begin{figure*}
  ...
  \begin{minipage}[c]{0.49\textwidth}
    ...
    \caption{Similar to Figure~\ref{fig:clens_calibration_conv} but for
      $\rmn{TPC}_{\rmn{morph}}$.
      $P_{\rmn{tpc}}$ is the probabilistic output generated by ConvNet.}
...
As descrbied in Section~\ref{sec:convnet},
we train our ConvNet model by monitoring its performance on the validatoin set.
Once we have finished training our model,
...
the raw images, and the performance of ConvNet is therefore comparable to
...
and star completeness values as functions of $i$-band magnitude for the
integrated counts.
...
\begin{figure}
  ...
  \caption{Galaxy ompleteness and star purity as function of the $r$-band
...
\begin{figure}
  ...
  \caption{Galaxy completeness and star purity as functions of the $r$-band
...
Alhtough ConvNet performs worse than $\rmn{TPC}_{\rmn{morph}}$ on the SDSS
data, its performance is much closer to $\rmn{TPC}_{\rmn{morph}}$, as ConvNet
is able to learn the shape information automatically from the raw images.

In Figure~\ref{fig:sdss_mag}, we compare the galaxy completeness
and star purity values for ConvNet and $\rmn{TPC}_{\rmn{morph}}$ as
functions of $i$-band magnitude for the differential counts in the SDSS data.
...
respectively.
Compared to Figure~\ref{fig:clens_calibration_conv},
when our ConvNet is applied to the SDSS data set,
its calibration curve is not as well-calibrated as when it is applied to the
CFHTLenS data set. 
Although it is not straightforward to compare the two calibration curves by
visual inspection, Table~\ref{table:sdss_metrics} shows that the CAL metrics of
ConvNet is lower than that of TPC.
...
It is also likely that the performance will be improved by,
