...
\begin{figure}
  ...
  \caption{
    The color-color space distribution of stars and galaxies in the SDSS data
    set. It is easy to see that there is significant overlap between stars
    and galaxies in the color-color space. 
  }
...
We randomly split the objects into training, hold-out validation,
and blind test sets
...
\begin{figure}
  ...
  \caption{
    The color-color space distribution of stars and galaxies in the CFHTLenS data
    set.
  }
...
\textit{backpropagation} procedure, which is nothing more than an application of
...
\begin{align}
  ...
  \frac{ \partial L }{ \partial x_i } &=
    ...
    \frac{ \partial y_i }{ \partial x_i }.
\end{align}
Finally, once the gradients $\partial L / \partial x_i$ have been
computed, it is straightforward to compute the gradients for the weight,
\begin{equation}
\frac{\partial L}{\partial w_{ji}} = y_j \frac{\partial L}{\partial x_i}.
\end{equation}
...
(Equation~\ref{eq:cross_entropy_all}) on the validation set.
The architecture described in this section provides the best performance on the validation data.
...
and finally use Equation~\ref{eq:posterior_bmc} to compute
...
and we show in Section~\ref{sec:results_and_discussion}
...
cores of Xeon E5 processors, it takes 23 minutes to train 500 trees,
for a total of 25 CPU hours.

\section{Results and Discussion}
  \label{sec:results_and_discussion}

In this section, we first describe the performance metrics we use to evaluate
the models. We then present the classification performance of our ConvNet model
on CFHTLenS and SDSS data sets and compare it with the performance of TPC.
...
After we use the training set to train our ConvNet model descrbied in
Section~\ref{sec:convnet} and the validation set to optimize the
hyperparameters, we evaluate the model's performance on the blind test set.
...
Thus, magnitudes and colors alone are not be sufficient to separate stars from
galaxies,
...
the raw images, and ConvNet is comparable to $\rmn{TPC}_{\rmn{morph}}$,
...
recognizable in the first convolutional layers.
...
Figure~\ref{fig:sdss_calibration_conv} and \ref{fig:sdss_calibration_tpc}
...
\href{https://github.com/EdwardJKim/dl4astro}{https://github.com/EdwardJKim/dl4astro}.  
...
We also note that using more training images will require the use of multi-GPU,
