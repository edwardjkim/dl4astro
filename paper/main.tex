...
\title[Star-galaxy Classification Using ConvNets]{Star-galaxy Classification Using Convolutional Neural Networks}

\author[E. J. Kim and R. J. Brunner]{
  Edward J.~Kim$^1$\thanks{jkim575@illinois.edu} and Robert J.~Brunner$^{2}$ \\
...
The latest advances in machine learning using deep convolutional neural networks
allow a machine to automatically learn the features directly from data,
minimizing need for input from human experts.
We present a star-galaxy classification framework that uses
a deep convolutional neural network (ConvNet) directly on the raw pixel values.
Using data from the Sloan Digital Sky Survey (SDSS) and
the Canada-France-Hawaii Telescope Lensing Survey (CFHTLenS),
we demonstrate that ConvNet is able to produce accurate and well-calibrated
probabilistic classifications that are competitive with
conventional machine learning techniques.
Future advances in deep learning may bring more success in currently ongoing and
...
very little feature engineering by hand.
\end{abstract}
...
However, ideally, the probability estimates themselves would be retained for all sources
...
which has become a core part of the astronomical image processing software
...
to transform the raw pixel values of an image to suitable features,
such as magnitudes or shape information of an object.
In a branch of machine learning called deep learning,
features are not designed by human experts;
they are learned from data by deep neural networks.
...
but it has never been applied to the problem of source classification.
...
from SDSS and Canada-France-Hawaii Telescope Lensing Survey (CFHTLenS).
...
and demonstrate that our ConvNet model is able to produce accurate and
well-calibrated probabilistic classifications with very little feature
engineering by hand.
In Section~\ref{section:data}, we describe the data sets used in this paper
and the pre-processing steps for preparing the image data sets.
We provide a brief overview of deep learning and ConvNets
in Section~\ref{section:deep_learning}, and
discuss our strategy for preventing overfitting in Section~\ref{section:overfitting}.
In Section~\ref{section:tpc}, we describe a state-of-the-art tree-based
...
Section~\ref{section:results_and_discussion}, and
we outline our conclusions in Section~\ref{section:conclusions}.
...
The resulting catalog contains photometry of over 300 million stars galaxies
...
server~\citep{li2008casjobs}~\footnote{http://skyserver.sdss.org/casjobs/}.

Using the CasJobs server, we randomly select 65,000 stars and galaxies.
...
galactic extinction corrections in magnitudes are applied
...
We randomly split the objects into training, validation, and test sets
of 40 000, 10 000, and 15 000, respectively.
We note that we perform a blind test, and the test set is not used in any
to train or calibrate the algorithms.
...
to that described in Section~\ref{section:sdss}.
Finally, images are downscaled to $48\times48$ pixels
to reduce reduce computational cost.

In the end, we have 8 545 stars and 57 843 galaxies available
for the training and testing processes.
We randomly split the objects into training, validation, and test sets of size
40 000, 10 000, and 13 278, respectively.
...
\begin{figure*}
  ...
  \caption{
    ...
    of two neurons and one output layer with two neurons.
    }
\end{figure*}
...
Let $\mathbf{x}$ be a vector of inputs to a given neuron,
$\mathbf{w}$ be a vector of weights, and
$\mathbf{b}$ be the bias.
...
has frequently been used as activation function, but it has recently fallen out of favor,
and the hyperbolic tangent function, $\sigma(x)=\tanh(x)$ is almost always used instead.
...
To mitigate this problem, we use leaky ReLUs that have a small negative slope
...
outperformed computers: speech recognition, language processing, and image classification.

The goal of learning is to find a set of weights and biases such that
the output from the network $\mathbf{y}=\left\{ y_1, y_2, \dots, y_n \right\}$
approximates the desired output
$\hat{\mathbf{y}}=\left\{\hat{y}_1, \hat{y}_2, \dots, \hat{y}_n \right\}$
as closely as possible for all input $\mathbf{x}$.
We can formulate this as the minimization of a loss function $L(\mathbf{x},\hat{\mathbf{y}})$
over the training data.
In this work, we use \textit{cross-entropy},
\begin{equation}
  L(y, \hat{y}) = -\hat{y} \log_2 y - (1 - \hat{y}) \log_2(1 - y),
\end{equation}
where $\hat{y}$ is the actual truth value (\eg 0 or 1)
and $y$ is the probability prediction made by the model.
We compute the loss function by taking the average of all cross-entropies in the sample.
Given $N$ samples, the loss function becomes
\begin{equation}
  L(y, \hat{y}) = - \frac{1}{N} \sum_{i=1}^{N} \hat{y}_i  \log_2 y_i
    + (1 - \hat{y}_i)  \log_2 (1 - \hat y_i).
\end{equation}
where $\hat{y}_i$ is the truth value and probability estimate of the $i$-th data,
respectively.

To find the weights $\mathbf{w}$ and biases $\mathbf{b}$ which minimize the loss,
we use a technique called \textit{gradient descent},
where we the following rules to update the parameters in each layer $j$:
\begin{align}
  \mathbf{w}_j &\rightarrow
  \mathbf{w}^{\prime}
  = \mathbf{w}_j - \eta \frac{\partial L}{\partial \mathbf{w}_j} \\
  \mathbf{b}_j &\rightarrow
  \mathbf{b}^{\prime}
  = \mathbf{b}_j - \eta \frac{\partial L}{\partial \mathbf{b}_j},
  \label{eq:gradeitn_descent}
\end{align}
...
Equation~\ref{eq:gradeitn_descent} for each mini-batch separately instead of
...
and the model that minimizes the loss is chosen as the best model.
...
with a sum of convolutions. The $k$-th feature map is given by
\begin{equation}
\mathbf{y}^k = \sigma \left( \sum_{j} \mathbf{w}_j^k \ast \mathbf{x}_j + b^k \right),
\end{equation}
where we sum over the set of filters,
$\ast$ is the convolution operator, and $\mathbf{w}_j^k$ represent the filters.
...
\begin{table*}
  \centering
  \caption{Summary of hyperparameters. Note that pooling layers have no learnable parameters.}
...
The output of the final fully-connected layer is fed to a softmax function.
The softmax function is given by
\begin{equation}
  P (y_i \mid \mathbf{x}) = 
    \frac{ e^{ \mathbf{x} \cdot \mathbf{w}_{y_i} } }
    { \sum_{k} e^{ \mathbf{x} \cdot \mathbf{w}_k } },
\end{equation}
where we interpret its output as the posterior probability that an object is a galaxy
(or a star).
...
In addition to hyperparamter optimization, we have explored more than 100 different architectures
and have found that the architecture described in this section provides
the best performance on the validation data.
For example, the architecture of \cite{krizhevsky2012imagenet}
...
We use a slightly positive value $b=0.1$ for all biases.
...
We note that our network takes about 40 hours to train the networks on a NVIDIA Tesla K40 GPU.

\section{Reducing Overfitting}
  \label{section:overfitting}
  
Our convolution neural network has $11\times10^6$ learnable parameters,
while there are only $5\times10^4$ images in the training set.
As a result, the model is very likely to \emph{overfit} without regularization.
In this section, we describe the techniques used to combat overfitting.

\subsection{Data Augmentation}
  \label{sec:data_augmentation}
  
One common method to combat overfitting is to artificially increase
the number of training data by using label-preserving transformations.
...
Although the data augmentation step~\ref{sec:data_augmentation} should
make our ConvNet model invariant to these types of transformations,
we find that applying BMC still results in a significant decrease in validation error.
...
We train two TPC models on the SDSS data set using different sets of
attributes.
...
($u$, $g$, $r$, $i$, $z$) and their corresponding colors
($u-g$, $g-r$, $r-i$, $i-z$).
...
and PSF magnitude,
\ie $\rmn{concentration}\equiv\rmn{psfMag}-\rmn{cModelMag}$.
The SDSS pipeline uses a parametric method based on the concentration,
\ie an object is classified as a galaxy if $\rmn{concentration}>0.145$.
...
and we show in Section~\ref{section:results_and_discussion}
...
\texttt{A\_WORLD} (the semi-major axis), \texttt{B\_WORLD} (the semi-minor axis)
for morphological features, in addition to
the five magnitudes and their corresponding colors,
...
our ConvNet model and compare it with the performance of TPC.
...
The simplest way to way to use the probability output is to
...
\eg $p_\rmn{cut} = 0.5$ \citep[\eg][]{henrion2011bayesian, Fadely2012},
...
\begin{equation} \label{eq:mse}
  \rmn{MSE} = \frac{1}{N} \sum^{N}_{i=1}
  \left( y_i - \hat{y}_i \right)^2,
\end{equation}
The MSE can be considered as both a score function that quantifies
how well a set of probabilistic predictions are calibrated,
or a loss function.
...
(see Figures~\ref{fig:clens_calibration_conv} and fig:\textcolor{red}{xxx}).

The problem with binning approach is either too few or too many bins
...
put the first 1000 elements in the first bin.
...
We then repeat this for the second bin (2 to 1001),
the third bin (3 to 1002), and so on, and average the binned calibration errors.
Thus, the overall calibration error is given by
\begin{equation}
\rmn{CAL} = \frac{1}{N-s} \sum_{b=1}^{N-s} \sum_{i=b}^{b+s-1}
\bigg| P_{\rmn{class},i} - \frac{\sum_{i=b}^{b+s-1} P_{\rmn{gal},i}}{s} \bigg|
\end{equation}
where $s=1000$ is the bin length.
...
For a perfect classifier, we can count the number of galaxies by summing
the values of $P_{\rmn{class}}$.
Thus, we measure the reliability of the classifier output by the absolute error
in the estimation of number of galaxies,
\begin{equation}
  \frac{\mid \Delta N_g \mid}{N_g} =
  \frac{\big| N_g - \sum_{i=1}^{N} P_{\rmn{class}} \big|}{N_g}
\end{equation}
...
\begin{figure}
  ...
  \caption{Galaxy completeness and star purity values as a function
           of the $i$-band magnitude
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figures/clens_calibration_conv.pdf}
  \caption{
    Calibration curve for ConvNet as applied to the CFHTLenS data set.
    $p_{\rmn{gal}}$ is the fraction of objects that are galaxies.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figures/clens_calibration_tpc.pdf}
  \caption{Calibration curve for TPC as applied to the CFHTLenS data set.}
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{
    (a) A sample $44\times44$ RGB image of a CFHTLenS galaxy.
    The RGB image is created by mapping R $\rightarrow g$ band magnitude,
    G $\rightarrow r$ band magnitude, and B $\rightarrow i$ band magnitude.
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{
    Same as Figure~\ref{fig:galaxy_activations} but for a CFHTLenS star.
    }
  \label{fig:star_activations}
\end{figure*}

We present in Table~\ref{table:clens_metrics} a summary of the results obtained
by applying ConvNet and TPC on the CFHTLenS data set.
...
It is not surprising that $\rmn{TPC}_{\rmn{phot}}$ performs significantly worse
than both ConvNet and $\rmn{TPC}_{\rmn{phot}}$,
as morphology is critical in separating stars from galaxies.
This suggests that ConvNet is able to learn the morphological features
automatically from the images.

In Figure~\ref{fig:clens_mag}, we compare the galaxy completeness
and star purity values for ConvNet and TPC as functions of $i$-band magnitude
for the differential counts.
We use the kernel density estimation~\citep[KDE][]{silverman1986density}
with the Gaussian kernel.
As the first panel shows, KDE is able to smooth the fluctutions in the distribution
without binning.
While ConvNet shows a slightly better performance than $\rmn{TPC}_{\rmn{morph}}$
in galaxy purity, the star completeness plots for ConvNet and TPC
are nearly identical.
Again, $\rmn{TPC}_{\rmn{phot}}$ performs significantly worse than both
ConvNet and TPC, and this suggests that ConvNets are able to learn
the shape information automatically from the images.
We note that, at these operating conditions ($c_g=0.96$, $p_s=0.97$),
both ConvNet and $\rmn{TPC}_{\rmn{morph}}$ outperform the
star-galaxy classification provided by
the CFHTLenS pipeline~\cite{hildebrandt2012cfhtlens} across all magnitudes.
We do not show the CFHTLenS classifications to avoid cluttering the plots.

In Figure~\ref{fig:clens_integrated}, we show the overall galaxy purity
and star completeness values as functions of magnitude for the integrated
counts.
ConvNet is able to mantain a galaxy purity of 0.9972 up to $i\sim24.5$,
...
ConvNet outperforms $\rmn{TPC}_{\rmn{morph}}$ or is similar to
$\rmn{TPC}_{\rmn{morph}}$ in the remaining regions.
...
for the probabilistic output of TPC,
...
of ConvNet is about 20\% lower than that of TPC.
...
When highly optimized features (\eg the $\rmn{concentration}$ of SDSS)
are not readily available from the catalog, a ConvNet model becomes more
attractive.
...
\begin{figure}
  ...
  \caption{Completeness and purity in the SDSS
           as a function of the $r$-band magnitude for
...
\begin{figure}
  ...
  \caption{Completeness and purity in the SDSS
           as a function of the $r$-band magnitude
...
\begin{figure}
  ...
  \caption{Similar to Figure~\ref{fig:sdss_mag}
           but as a function of $g-r$ color.
           The bin size of histogram in the top panel is 0.05.}
  \label{fig:sdss_g_r}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/sdss_calibration_conv.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/sdss_calibration_tpc.pdf}
  \caption{Calibration curve for TPC as applied to the CFHTLenS data set.}
...
In contrast with the CFHTLenS data set in the previous section,
it is apparent that $\rmn{TPC}_{\rmn{morph}}$ outperforms ConvNet
in all metrics except the $\rmn{CAL}$.
Both ConvNet and $\rmn{TPC}_{\rmn{morph}}$ still outperforms
$\rmn{TPC}_{\rmn{phot}}$ in all six metrics by a significant amount.

In Figure~\ref{fig:sdss_mag}, we compare the galaxy completeness
and star purity values for ConvNet and TPC as functions of $i$-band magnitude
for the differential counts.
We note that $\rmn{TPC}_{\rmn{morph}}$ outperforms the star-galaxy
classifier used by the SDSS pipeline (\ie an object is classifed as a galaxy
if the concentration parameter is greater than 0.145) across all magnitudes.
We do not show the SDSS classifications to avoid cluttering the plots.
While ConvNet shows a similar but slighly worse performance than
$\rmn{TPC}_{\rmn{morph}}$,
...
and star completeness values as functions of magnitude for the integrated
counts.
ConvNet is able to mantain a galaxy purity of 0.9915 up to $i\sim22.5$,
while $\rmn{TPC}_{\rmn{morph}}$ provides a glaxay purity of 0.9977.
...
Figure~\ref{fig:sdss_calibration_conv} shows the calibration curve
that compares $P_{\rmn{gal}}$,
the fraction of objects that are galaxies (as determined from their spectra)
to $P_{\rmn{conv}}$ the probabilistic output that our ConvNet model produces.
The calibration curve is nearly diagonal, which implies that ConvNet
is well-calibrated and we can treat $P_{\rmn{conv}}$ as the probability
that an object is a galaxy.
Figure~\ref{fig:sdss_calibration_tpc} shows the calibration curve
for the probabilistic output of TPC,
which is apparently not as well-calibrated as ConvNet.
These calibration curves visually confirm the results in
Table~\ref{table:sdss_metrics} that the calibration error
of ConvNet is about 20\% lower than that of TPC.
...
In the interest of scientific reproducibility, we make all our code available at
...
The major advatange of ConvNets is that useful features are learned
automatically from raw pixel values, while traditional machine learning
algorithms require feature engineering as a separate process
to provide produce accurate classifications.
...
as a good feature extractor for identifying images of cats would of little use
for classifying sailboats, and it is impractical to design a feature extractor
for each class.
However, if there already exists a good feature extractor for the problem
at hand, \eg the concentration parameter or the \texttt{SExtractor} software,
conventional machine learning algorithms that are proven to be effective,
...
For the CFHTLenS data set, our ConvNet model outperforms TPC trained on the
output of \texttt{SExtractor}.
Since the SDSS catalog provides the concentration paramter that is highly
optmized for star-galaxy classification, TPC works better for SDSS.
...
with spectroscopic follow-up, as the performance of ConvNets apparently
...
become an attractive option because it can be applied immediately on raw images
...
The winning solution of \cite{dieleman2015rotation}
...
It is likely that the performance will be improved by,
not only training multiple network architectures,
but also combining with different star-galaxy classifiers.
