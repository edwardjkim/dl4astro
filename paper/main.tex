...
\newcommand{\RC}[1]{{\color{red}\textbf{Robert Comment: #1\ }}}
...
\RC{It might be helpful to have a colorbar with these plots to show relative densities. I also wonder if the extra whitespace might be reduced somehow. Finally, I actually think a ratio plot might be best her, where the hexbin shows the ratio of the normalized counts.}
\textcolor{magenta}
{
I added colorbars. I don't see how I can reduce the whitespace further. I tried creating a ratio plot (see \url{https://github.com/EdwardJKim/dl4astro/blob/master/paper/figures/sdss_color_color.test.pdf}) but I think I prefer separate color-color diagrams for stars and galaxies, because the only reason I included these color-color diagrams is the referee will be an astronomer, and I think a ratio plot will be less familiar to the referee than the conventional plots with number counts. If you think a ratio plot is better, I can polish the linked ratio plot and replace the color-color diagrams with ratio plots.
}
...
\RC{Be useful to state what the ratio of Stars to galaxies are in these three data sets (train, validate, test). Also, a referee will likely ask how they are distributed in magnitude and color.}
\textcolor{cyan}{
...
\RC{ Again the S:G ratio in the data sets and mag and color distributions. Referee will likely ask, so you can put something in here to simplify review process.}
\textcolor{cyan}{
...
\begin{table*}
  \centering
  \caption{Summary of ConvNet architecture and hyperparameters. Note that pooling layers have no learnable parameters. \RC{ This might be much easier to understand if it is a flowchart, like we did in INFO490 with Keras. Is that possible? I say this as it would better  match your toy ANN in  a previous figure.}
\textcolor{magenta}{
Please see \url{https://github.com/EdwardJKim/dl4astro/blob/master/paper/figures/layers.png}. I can make this one look better and include it, but I would rather not include it if you are OK with it, because I don't think it adds any useful information. It seems to me the flowchart is basically each row in the below table connected with arrows.
}
  }
...
\RC{ I wonder why? Is this somehow relevant?}
\textcolor{magenta}{ I don't know. I treated regression vs classification problem as just another hyperparamter I have to choose. Using classification mode + softmax was simply a better hyperparamter for the problem.
}
...
\RC{You do not talk anywhere on the software you actually use to do this work. You should introduce Python, Lasagna, Theano, and any other relevant software. You also should cite them. Finally, what hardware was used for these tests?}
\textcolor{cyan}{
...
\RC{Why 1000 here?}
\textcolor{magenta}{It's somewhat arbitrary.  Number of ojbects in the test set / Number of bins in the calibration curve = 15000 / 10 = 1500.  So about 1000, rounded down.  But I could have chosen 100 or 10000 or any other number. Do you think I need a justification here?}
...
\begin{figure*}
  ...
  \caption{
    ...
    \RC{Note that usually R = Red (i-band), G = Green (r-band), and B = Blue (g-band)}
    \textcolor{magenta}{Fixed}
    }
...
\RC{I wonder if augmentation hurts us here, since we are populating our training data with more faint, and confusing objects? Mag distribution of stars and galaxies might help clarify this point.}
\textcolor{magenta}{
Even if we are populating our training data with fainter objects, augmentation still improves the overall performance on the validation set, so I don't think augmentation hurts us.
}
